...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!(commands.isEmpty()) && !(_commands.isEmpty())) {
            throw new IllegalStateException("error: illegal command argument");
        } else {
    ...
    private void checkFinishedDependencies() {
        for (int succ : _depends.successors(_vertex)) {
            if (_depends.getLabel(succ).isUnfinished()) {
                error("error: Unfinished dependencies.");
            }
        }
    }
        ...
        if (!_finished) {
            if (_time == null) {
                ...
                for (int i : _depends.successors(_vertex)) {
                    if (_time < _depends.getLabel(i).getTime()) {
                        return true;
                    }
                }
            }
    ...
    void rebuild() {
        ...
        if (outOfDate()) {
            ...
            for (String myCommands : _commands) {
